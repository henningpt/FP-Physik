...
A=np.genfromtxt('4b_5_12_alternating.dat', unpack=True)
X=[]
print(len(A[0,:]))
print(A[1,0])
print(A[1,1])
...
for i in range(len(A[1,:])-r+1):
    ...
    if A[0,i+r/2]==10040:
        X=X+[A[:,i+r/2-1]]          
    #regulaerer Ablauf
    if A[0,i+r/2-1]<3000 and A[1,i+r/2-1]>f_abs_3000 and not A[0,i+r/2-1]==800 and not A[0,i+r/2-1]==150 and not A[0,i+r/2-1]==280 :
...
plt.plot(k[1:12],Y[0,0:11],'rx')
plt.plot(k[0:12],Y[0,11:23][::-1],'rx')
plt.plot(k[1:11],Y[0,23:33],'rx')
plt.plot(k[2:10],Y[0,33:41][::-1],'rx')

plt.xlabel('Wellenzahl k in 1/m')
plt.ylabel('Resonanzfrequenzen in Hz')
plt.legend(loc="best")
#plt.show()
plt.savefig('4b_5_alt_red.jpg')
