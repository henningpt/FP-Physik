...
A=np.genfromtxt('4b_7_1b.dat', unpack=True)
X=[]
print(len(A[0,:]))
print(A[1,0])
print(A[1,1])
...
for i in range(len(A[1,:])-r+1):
    ...
    #   X=X+[A[:,i+r/2-1]]          
    #regulaerer Ablauf
    if A[0,i+r/2-1]<8500 and not A[0,i+r/2-1]==800 and not A[0,i+r/2-1]==2910 and not A[0,i+r/2-1]==280 :
...
plt.plot(k[1:12],Y[0,11:22][::-1],'rx')
plt.plot(k[1:10],Y[0,22:31],'rx')

plt.xlabel('Wellenzahl k in 1/m')
plt.ylabel('Resonanzfrequenzen in Hz')
plt.legend(loc="best")
#plt.show()
plt.savefig('4b_7_1b.jpg')
